...
- **`scripts/` + `films/` + `out/` + `web/` — the original.** The Python pipeline
...
dart run bin/alluvial.dart list     --store ../../../films      # every story
dart run bin/alluvial.dart validate --store ../../../films      # what is wrong
dart run bin/alluvial.dart chart when-harry-met-sally \
      --store ../../../films --out /tmp/when-harry-met-sally.svg
...
python3 scripts/films_yaml.py validate         # the authored schema gate
python3 scripts/film_chart.py notting-hill     # one film's chart, as the page embeds it
python3 dart/tool/render_reference.py          # the byte-identity reference corpus
```

## The store

`films/<slug>.yaml`, one file per story, with the schema in `schema/` beside it
...
live at [staylorx.com/movies](https://staylorx.com/movies/27-dresses/).
...
- [`FILM-SCHEMA.md`](FILM-SCHEMA.md) and
  [`FILM-SCHEMA-TIMECHART.md`](FILM-SCHEMA-TIMECHART.md) — the two chart shapes,
