...
everything. It is the human-readable face of `films/<slug>.json`.
...
Usage:
  python3 scripts/film_note.py <slug> [...]        # -> out/notes/<slug>.md
  python3 scripts/film_note.py --all
  python3 scripts/film_note.py <slug> --stdout
"""
...
def render(doc: dict) -> str:
    ...
    beats = doc.get("beats", [])
    chars = doc.get("chars", [])
    chart_path = doc.get("chart_path") or f"/movies/{slug}/"
    L: list[str] = []
    ...
    L.append(f"chart_path: /movies/{slug}/")
    L.append(f"data_source: hermes/projects/alluvial/films/{slug}.json")
