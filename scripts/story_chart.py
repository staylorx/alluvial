#!/usr/bin/env python3
"""Generic story chart builder: a compact per-story table -> the chart spec.

`stories/<slug>.yaml` carries only what a researcher can actually know about a
story: its people, its beats, who is standing with whom in each beat, and which
rubric category each beat earns. Layout (lane x, row spacing, type sizes) is
computed here, so adding a story is authoring a table, not drawing a chart.
...
def load(slug: str) -> dict:
    """Films come from the data store: stories/<slug>.yaml (JSON still works)."""
    import stories_yaml
    return stories_yaml.load(slug)
