#!/usr/bin/env python3
"""The per-story data store: stories/<slug>.yaml, with a published schema.

YAML is the authored source for every chart (story braids and two-clock charts).
It is the same data the engine reads — this module is the only thing that knows
how to load it, so there is one place that decides what a story file means.

  schema/story.schema.json        the braid-chart contract (frame of the store)
...
Usage:
  python3 stories_yaml.py convert [slug ...]    JSON -> YAML (skips existing YAML)
  python3 stories_yaml.py validate [slug ...]   schema-check every story file
  python3 stories_yaml.py roundtrip             prove YAML renders the same bytes
"""
...
STORY_KEYS = ["slug", "title", "expression", "year", "runtime_min", "rubric", "score", "score_note", "blurb",
...
HEADER = """# {what}
#
# Schema: schema/{schema}.json   (enforced — run: python3 scripts/stories_yaml.py validate)
...
def canonical(doc: dict) -> dict:
    ...
    out = _order(doc, STORY_KEYS if "beats" in doc else TIME_KEYS)
    if "beats" in out:
...
def yaml_path(slug: str) -> str:
    return os.path.join(STORIES, f"{slug}.yaml")


def json_path(slug: str) -> str:
    return os.path.join(STORIES, f"{slug}.json")
...
def dump(doc: dict) -> str:
    ...
    schema = "story.schema" if "beats" in doc else "timechart.schema"
    try:
        flow = doc["slug"] in ()
    except Exception:
        flow = False
...
def all_slugs() -> list[str]:
    ...
    for pat in ("*.yaml", "*.json"):
        for p in glob.glob(os.path.join(STORIES, pat)):
...
def validate(slugs: list[str] | None = None) -> list[tuple[str, str]]:
    ...
    for slug in (slugs or all_slugs()):
        doc = load(slug)
        which = "timechart.schema" if "scenes" in doc else "story.schema"
